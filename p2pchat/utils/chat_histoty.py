...
def print_in_constant_place(*args,key=None,ending_line=None):
    ...
    text = ' '.join([str(arg) for arg in args])
    if key is None:
        print_and_remember("key must be provided to constant messages",ending_line=ending_line)
    new_history=[]
    found=False
    for message in history.history:
        if not (isinstance(message,tuple) and len(message)==3 and message[1:3] == ('constant',key)):
