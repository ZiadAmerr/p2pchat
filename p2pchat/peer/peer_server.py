...
from p2pchat.utils.chat_histoty import history,print_and_remember,print_volatile_message
...
class PeerTCPManager(SockerManager):
    ...
    def handle_new_chat_request(self,request):
        ...
        if a=='y':
            ...
            not_chatting.set()
            response= S4P_Response.RCPNTREF(f"your private room request has been rejected ")
        ignore_input.clear()
        return response
    ...
    def handle_join_room_request(self,request):    
        ...
        if not request or not utils.validate_request(request.get('body',{}),['sender','chatroom_key']):
        ...
        ignore_input.set()
        a=input(blue_text(f"{sender.get('username')} wants to join {chatroom_key}? (y/n): "))
        while(a not in ['y','n']):
        ...
        if a=='y':
            ...
            if response.get('body').get('is_success'):
                print_volatile_message(green_text(f"{sender.get('username')} was added to chatroom \"{chatroom_key}\" successfully."))
                res= S4P_Response.JOINEDRM(f"your request was accepted",{"key":chatroom_key})
            else:
                print_volatile_message(red_text("couldn't admit user to chatroom."))
                res= S4P_Response.UNKWNERR(f"Couldn't join chatroom, try again later, or may be you are already registerd")
            time.sleep(2)
...
class PeerUDPManager(SockerManager):
    ...
    def handle_send_message_request(self,request):        
        ...
        if not request or request.get("body", {}).get("key")!=ClientAuth().current_chatroom:
            print(red_text("Invalid Request, no or invalid key"),request)
            return S4P_Response.INCRAUTH("Invalid Request, no or invalid key")
        ...
        if message=='exit_':
            ...
            return S4P_Response.MESGSENT(f"message recieved ;)",None)
        print_and_remember(f"{yellow_text(f'{sender_name} >> ')} {message}",ending_line="you >> ")
        return S4P_Response.MESGSENT(f"message recieved",None)
    def start_socket(self):
        pass
    def deactivate(self):
